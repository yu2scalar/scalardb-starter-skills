...
|---|---|---|---|
| 1 | `scalardb-set-k8s-env` | Select a Kubernetes environment (AKS / EKS / GKE / minikube), switch kubectl context, and verify connectivity. Starts minikube for you if needed; cloud clusters must already exist. | planned |
| 2 | `scalardb-start-sample-db` | Start demo PostgreSQL + MySQL databases in-cluster (credentials `scalaradmin` / `scalaradmin` — **demo use only**). | planned |
...
| 8 | `scalardb-stop-sample-db` | Stop the demo databases (optionally keeping their data volumes). | planned |
